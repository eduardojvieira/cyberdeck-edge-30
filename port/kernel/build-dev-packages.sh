...
readonly VERSION=5.10.209+git20240821.bd42a1bb-1~eqsdev1
...
for tool in docker git sha256sum cmp find grep stat; do command -v "$tool" >/dev/null || fail "missing host tool: $tool"; done
...
for run in one two; do
  ...
  docker run --rm --user "$(id -u):$(id -g)" \
    ...
    --workdir /build "$IMAGE" /bin/bash -euo pipefail -c '
      ...
      grep -Fxq CONFIG_LTO_CLANG_THIN=y "$out/.config"; grep -Fxq CONFIG_CFI_CLANG=y "$out/.config"
      ...
      find "$out" -type f -name "*.ko" -print0 | while IFS= read -r -d "" module; do rel=${module#"$out/"}; install -D -m 0644 "$module" "$module_dir/kernel/$rel"; done
      ...
      [[ $(find "$module_dir" -type f -name "*.ko" | wc -l) == 333 ]] || { echo wrong-module-count >&2; exit 1; }
      ...
      cp "$dt/dtbo.img" "$boot_dir/dtbo.img"; cp "$dt/dtb.img" /package/dtb.img; cp /common/modules.load.vendor_boot /package/vendor/lib/modules/modules.load
      cmp /common/modules.load.vendor_boot /package/vendor/lib/modules/modules.load
      [[ $(sort -u /package/vendor/lib/modules/modules.load | wc -l) == 99 ]]
      [[ $(wc -l < /package/vendor/lib/modules/modules.load) == 101 ]]
      while IFS= read -r name; do src=$(find "$module_dir" -type f -name "$name" -print -quit); [[ -n "$src" ]] || { echo "vendor-module-missing:$name" >&2; exit 1; }; install -m 0644 "$src" "/package/vendor/lib/modules/$name"; install -m 0644 "$src" "/package/vendor-meta/lib/modules/$release/$name"; done < <(sort -u /package/vendor/lib/modules/modules.load)
      ...
      [[ $(find /package/vendor/lib/modules -maxdepth 1 -name "*.ko" | wc -l) == 99 ]]
      ...
      ! find /package/vendor/lib/modules -mindepth 1 -type d -print -quit | grep -q .
      ...
      mkbootimg --header_version 4 --pagesize 4096 --kernel "$image" --ramdisk "$initramfs" --cmdline "printk.devkmsg=on firmware_class.path=/data/vendor/param/firmware" --output "$boot_dir/boot.img"
      ...
      mkbootimg --header_version 4 --pagesize 4096 --dtb /package/dtb.img --vendor_bootconfig /package/vendor_bootconfig --ramdisk_type platform --ramdisk_name "" --vendor_ramdisk_fragment /package/vendor-ramdisk.lz4 --vendor_boot "$boot_dir/vendor_boot.img"
...
CONFIG
      ...
      unpack_bootimg --out /package/unpack-boot --boot_img "$boot_dir/boot.img" | grep -Fxq "boot image header version: 4"
      unpack_bootimg --out /package/unpack-boot --boot_img "$boot_dir/boot.img" | grep -Fxq "command line args: printk.devkmsg=on firmware_class.path=/data/vendor/param/firmware"
      head -c 8 "$boot_dir/vendor_boot.img" | grep -Fxq VNDRBOOT
      unpack_bootimg --out /package/unpack-vendor --boot_img "$boot_dir/vendor_boot.img" > /package/vendor-unpack.txt
      grep -Fxq "vendor boot image header version: 4" /package/vendor-unpack.txt
      grep -Fxq "page size: 0x00001000" /package/vendor-unpack.txt
      ...
      grep -Fxq androidboot.hardware=qcom /package/vendor_bootconfig
      grep -Fxq androidboot.memcg=1 /package/vendor_bootconfig
      grep -Fxq androidboot.usbcontroller=a600000.dwc3 /package/vendor_bootconfig
      lz4 -d -q -c /package/vendor-ramdisk.lz4 | cpio -it | sed "s#^\\./##" | grep -Fxq lib/modules/modules.load
      lz4 -d -q -c /package/vendor-ramdisk.lz4 | cpio -it | sed "s#^\\./##" | grep -Fxq lib/modules/qcom_wdt_core.ko
      avbtool info_image --image "$boot_dir/boot.img" | grep -Eq "Partition Name:.*boot$"
      avbtool info_image --image "$boot_dir/vbmeta.img" | grep -Eq "Flags:.*3$"
      ...
      grep -Fq ./boot/Image- /package/image.list
      ! grep -Eq "boot/(boot|vendor_boot|dtbo|vbmeta)\\.img" /package/image.list
      grep -Fq ./boot/boot.img /package/bootimage.list
      dpkg-deb -f /package/linux-bootimage-motorola-eqs_*.deb Depends | grep -Fq "linux-image-motorola-eqs (= $version), flash-bootimage"
      ...
      [[ -f "$config" ]]
      grep -Fxq FLASH_ENABLED=no "$config"
      grep -Fxq DEVICE_HAS_VENDORBOOT_PARTITION=yes "$config"
      grep -Fxq USERDATA_FLASHING_METHOD=fastboot "$config"
      grep -Fxq "EXTRA_INFO_DEVICE_IDS=\"eqs\"" "$config"
...
(cd "$artifacts" && sha256sum boot.img vendor_boot.img dtbo.img vbmeta.img *.deb > SHA256SUMS)
{
  ...
  printf 'config=ThinLTO+CFI\nmodules=333 (319 in-tree + 14 external)\n'
  printf 'cmdline=printk.devkmsg=on firmware_class.path=/data/vendor/param/firmware\n'
